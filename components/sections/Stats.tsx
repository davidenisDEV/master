        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-0 lg:divide-x divide-slate-200 dark:divide-slate-800">
            {siteConfig.stats.map((stat, index) => {
              // Os textos virão do arquivo de tradução (t.stats.items[index]) na próxima etapa
              const translationItem = t?.stats?.items?.[index] || { title: "", desc: "" };
              
              return (
                <div key={stat.id} className={`flex flex-col items-center text-center px-4 ${index !== 0 ? 'lg:pl-8' : ''} ${index !== siteConfig.stats.length - 1 ? 'lg:pr-8' : ''}`}>
